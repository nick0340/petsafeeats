...
// Note: In Next.js App Router, generateStaticParams would be used for static generation,
// but since this file has "use client", it will be dynamically rendered or we can extract the metadata/static generation
// to a server component layout or wrapper if needed later.
...
export default function PetPage({ params }: { params: { pet: string } }) {
  ...
  // If pet doesn't exist, redirect to home
  useEffect(() => {
    if (!allPets.find(p => p.id === pet)) {
      router.push('/');
    } else {
  ...
  const renderFoodGrid = (foods: FoodItem[], safety: 'safe' | 'caution' | 'toxic') => {
    ...
    return (
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
        {foods.map((food, i) => (
          <Link href={`/can-${pet}-eat-${food.slug}`} key={food.id} passHref legacyBehavior>
            <motion.a
              ...
              className={`card-soft ${sc.bg} rounded-xl p-5 text-center cursor-pointer block`}
  ...
  return (
    <div>
      <Breadcrumbs items={[{ label: petInfo.plural }]} />
      ...
      <section className="bg-gradient-to-b from-brand-light to-white section-spacing">
        <div className="container-main">
          <div className="max-w-[800px] mx-auto text-center">
            ...
            <SearchBar currentPet={pet} variant="hero" showTrending={false} />
          </div>
        </div>
      </section>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">
            {categories.map((cat) => (
              <Link href={`/category/${cat.id}`} key={cat.id} passHref legacyBehavior>
                <a
                  className="card-soft rounded-xl p-5 text-center cursor-pointer group block"
