...
import { foodDatabase, getPetData, categories, type FoodCategory, type SafetyLevel, type PetType } from '../data/foods';
...
export default function AllFoods({ initialCategory }: AllFoodsProps) {
  ...
  const breadcrumbItems = initialCategory
    ? [{ label: 'All Foods', path: '/foods' }, { label: categories.find(c => c.id === initialCategory)?.name || initialCategory }]
    : [{ label: 'All Foods' }];
    ...
    <div>
      ...
      <div className="container-main pb-20">
        ...
        <div className="mb-10 max-w-[600px] mx-auto">
          <SearchBar currentPet={selectedPet} variant="sticky" showTrending={false} />
        </div>
          ...
          {filtered.map((food, i) => {
            ...
            return (
              <motion.button
                ...
                onClick={() => router.push(`/can-${selectedPet}-eat-${food.slug}`)}
